describe "Lyft::AccessToken" do
  let(:tok) {:dummy_access_token}
  let(:expires_in) { 74000 }
  let(:expires_at) { Time.utc(2016,1,1) }

  context "When just token is set" do
    subject {Lyft::AccessToken.new(tok)}
    it("has the token value") { expect(subject.token).to eq tok }
  end

  context "When only expires_in is set" do
    subject {Lyft::AccessToken.new(tok, expires_in)}
  ...
  context "When everything is set" do
    subject {Lyft::AccessToken.new(tok, expires_in, expires_at)}
