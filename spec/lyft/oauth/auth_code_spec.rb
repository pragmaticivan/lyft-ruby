...
describe "OAuth2 Auth Code" do
  ...
  context "When no redirect_uri is given" do
    before(:example) do
      Lyft.configure { |config| config.redirect_uri = nil }
    end

    let(:err_msg) { Lyft::ErrorMessages.redirect_uri }

    it "Throws an error" do
      expect {subject.auth_code_url}.to raise_error(Lyft::InvalidRequest, err_msg)
    end
  end
  ...
  context "When an auth code url is requested with no options" do
    ...
    it "Returns the client id in the uri" do
      expect(params("client_id")).to eq Lyft.config.client_id
    end
  ...
  context "When an auth code url is requested with options" do
    ...
    it "Returns the client id in the uri" do
      expect(params("client_id", opts)).to eq Lyft.config.client_id
    end
